...
export function HomePage({ nowMs, onOpenRaffle, onOpenSafety }: Props) {
  const { bigPrizes, endingSoon, stats, isLoading, items } = useHomeRaffles();

  // ✅ Only show skeletons on cold load (prevents flicker on background refresh)
  const hasAnyItems = (items?.length ?? 0) > 0;
  const showSkeletons = isLoading && !hasAnyItems;
      ...
      <div className="hp-container">
        ...
        <div className="hp-hero">
          ...
          <div className="hp-stats-bar">
            <div className="hp-stat-item">
              <div className="hp-stat-val">{showSkeletons ? "..." : stats.totalRaffles}</div>
              <div className="hp-stat-lbl">Raffles Created</div>
            </div>
            <div className="hp-stat-sep" />
            <div className="hp-stat-item">
              <div className="hp-stat-val">{showSkeletons ? "..." : fmtUsd(stats.settledVolume)}</div>
              <div className="hp-stat-lbl">Prizes Settled</div>
            </div>
            <div className="hp-stat-sep" />
            <div className="hp-stat-item highlight">
              <div className="hp-stat-val">{showSkeletons ? "..." : fmtUsd(stats.activeVolume)}</div>
        ...
        <div className="hp-podium-section">
          ...
          <div className="hp-podium">
            ...
            {/* ✅ Keep showing cached data even if isLoading flips true during refresh */}
            {!showSkeletons && podium.silver && (
              <div className="pp-silver-wrapper">
                <RaffleCard
        ...
        <div>
          ...
          <div className="hp-strip">
            ...
            {!showSkeletons && endingSoonSorted.length === 0 && (
