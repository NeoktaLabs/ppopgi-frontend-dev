...
interface RaffleCardProps {
  ...
  feeRecipient?: string;
  /**
   * Strict verified signal from parent.
   * Should already mean: official deployer AND registered typeId === 1.
   */
  isVerified?: boolean;
}
...
export function RaffleCard({
  ...
  const isStrictVerified = !!isVerified;
  ...
  return (
    <>
      <div
        className={`relative w-64 h-[22rem] rounded-[2rem] p-1.5 bg-gradient-to-br ${getTheme(color)} shadow-xl transform transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:scale-[1.02] border-2`}
      >
        {rank && (
