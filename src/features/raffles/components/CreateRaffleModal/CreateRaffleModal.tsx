...
import { ADDRESSES } from "../../../../shared/config/contracts";
...
import { createCreateRaffleModalStyles } from "./CreateRaffleModal.styles";
...
export function CreateRaffleModal({ open, onClose, onCreated }: Props) {
  ...
  const [, setApprovedOnce] = useState(false);
  ...
  // ----------------- STYLE -----------------
  const { overlay, modal, header, title, subtitle, closeBtn, body, secA, secB, secC, secD, secTitle, tinyToggle, inputLabel, tip, input, selectStyle, grid2, gridDuration, pill, pillGood, pillWarn, btnRow, btnPrimary, btnSecondary, btnDisabled, linkBtn, centeredNote, redCentered, addrLink } = createCreateRaffleModalStyles();

  // ---------- derived UI pills (pure UI; no behavior changes) ----------
  const balancePill = useMemo(() => {
    if (!me) return <span style={pill}>Signed out</span>;
    if (allowLoading || usdcBal == null) return <span style={pill}>Balance…</span>;
    return hasEnoughBalance ? <span style={pillGood}>Balance ✓</span> : <span style={pillWarn}>Low balance</span>;
  }, [me, allowLoading, usdcBal, hasEnoughBalance, pill, pillGood, pillWarn]);

  const approvalPill = useMemo(() => {
    if (!me) return null;
    if (allowLoading || allowance == null) return <span style={pill}>Allowance…</span>;
    return hasEnoughAllowance ? <span style={pillGood}>Approved ✓</span> : <span style={pillWarn}>Needs approval</span>;
  }, [me, allowLoading, allowance, hasEnoughAllowance, pill, pillGood, pillWarn]);

  // ---------- Preview raffle (best-effort shape for the card) ----------
  const previewRaffle = useMemo(() => {
    const now = Math.floor(Date.now() / 1000);
    const deadline = String(now + Math.max(60, durationSecondsN));

    return {
      id: createdRaffleAddr || "0xPREVIEW000000000000000000000000000000000000",
      name: name || "Preview raffle",
      status: "OPEN",
      deadline,
      prize: winningPotU.toString(),
      ticketPrice: ticketPriceU.toString(),
      sold: "0",
      minTickets: minTn,
      maxTickets: maxTickets.trim() === "" ? "0" : String(maxTn),
      creator: me || "0x0000000000000000000000000000000000000000",
    };
  }, [createdRaffleAddr, name, durationSecondsN, winningPotU, ticketPriceU, minTn, maxTickets, maxTn, me]);
