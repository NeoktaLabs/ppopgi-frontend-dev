...
import "../RaffleCard/raffleCard.css"; // reuse pp-rc-titleClamp + pp-rc-pulse
import { createRaffleDetailsModalStyles } from "./RaffleDetailsModal.styles";
...
import { cleanIntInput, clampInt, toInt } from "../../utils/numbers";
import { isZeroAddr, shortAddress as short } from "../../utils/address";
...
function joinBlockedReason(
  ...
  if (displayStatus === "Settled") return "This raffle is settled.";
  if (displayStatus === "Canceled") return "This raffle was canceled.";
  if (deadlinePassed) return "Deadline passed.";
  return "This raffle cannot be joined right now.";
}
...
export function RaffleDetailsModal({ open, raffleId, onClose }: Props) {
  ...
  const { overlay, card, notch, tearLine, topRow, statusChip, miniBtn, miniBtnDisabled, copyToast, titleWrap, smallKicker, titleText, prizeKicker, prizeValue, grid2, grid3, mini, miniLabel, miniValue, hint, section, panel, row, label, value, input, btnDisabled, btnEnabled, bottomRow, bottomText } = createRaffleDetailsModalStyles(status);
